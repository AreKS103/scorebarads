...
export function ImageUploadZone({ label, slot, minWidth, minHeight, aspectLabel, images, onChange }: ImageUploadZoneProps) {
  ...
  return (
    <div className="space-y-3">
      <div
        className={cn("flex min-h-40 flex-col items-center justify-center rounded-lg border border-dashed border-border bg-card p-5 text-center transition-colors", isDragging && "bg-muted/50")}
        onDragOver={(event) => {
      ...
      >
        ...
        <label className="mt-4 inline-flex h-9 cursor-pointer items-center gap-2 rounded-md border border-input bg-background px-4 py-2 text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground">
